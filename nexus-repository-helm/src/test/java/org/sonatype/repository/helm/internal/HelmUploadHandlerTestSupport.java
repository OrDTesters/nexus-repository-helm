...
public abstract class HelmUploadHandlerTestSupport
    extends TestSupport
{
  protected final String REPO_NAME = "helm-hosted";
  ...
  protected abstract UploadHandler newHelmUploadHandler(
  ...
  public void baseSetup() {
    when(contentPermissionChecker.isPermitted(eq(REPO_NAME), eq(HelmFormat.NAME), eq(BreadActions.EDIT), any()))
        .thenReturn(true);

    underTest = newHelmUploadHandler(contentPermissionChecker, new SimpleVariableResolverAdapter(), emptySet());
    when(repository.getFormat()).thenReturn(new HelmFormat());
  ...
  public void testGetDefinitionWithExtensionContributions() {
    ...
    underTest =
        newHelmUploadHandler(contentPermissionChecker, new SimpleVariableResolverAdapter(), getDefinitionExtensions());
    UploadDefinition def = underTest.getDefinition();
